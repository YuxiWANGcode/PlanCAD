...
if __name__ == '__main__':
    ...
    parser.add_argument('--llm_ckp_dir', type=str, default='meta-llama/Llama-3.1-1B', help='llm checkpoints dir')
    ...
    assert args.dataset in ['ETTh1', 'electricity', 'weather', 'traffic', 'foursquare', 'yj', 'us']
    if args.dataset == 'ETTh1':
        ...
        root_path = ''
        preprocess_foursquare_data(root_path, city=args.city, scale=False, size=[seq_len, label_len, pred_len], interval=None)
        data_set = Dataset_Preprocess_Foursquare(
            root_path='',
            city=args.city,
            size=[seq_len, label_len, pred_len])
    elif args.dataset == 'yj':
        root_path = ''
        ...
        data_sets = []
        for flag in flags:
            data_set = Dataset_Preprocess_US(
                root_path='',
                size=[seq_len, label_len, pred_len],
                city=args.city,
                flag=flag)
        ...
        data_set_token = Dataset_Preprocess_US_Token(
            root_path='',
            size=[seq_len, label_len, pred_len],
            city=args.city)
